...
describe Texter do
	...
	context '#send_confirmation' do
		it 'tells the client to create a text using twilio' do
			...
			expect(messages).to receive(:create).with({from: ENV['TWILIO_FROM'], to: ENV['TWILIO_TO'], body: "Thank you! Your order was placed successfully and will be delivered by 11:33"})
	...
	context '#check_for_orders' do
		...
		it 'tells the establishment to place an order using each inbound message not already deleted' do
			allow(messages).to receive(:list).with({to: ENV['TWILIO_FROM']}).and_return([message1,message2,message3])
		...
		it 'doesn\'t tell the establishment to place an order in a message already redacted' do
			...
			allow(messages).to receive(:list).with({to: ENV['TWILIO_FROM']}).and_return([message1])
