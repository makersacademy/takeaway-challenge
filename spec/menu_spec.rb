...
describe Menu do
  let(:dish_1) { double(:dish_1, name: 'Foo soup', price: 2, print_price: '£2.00') }
  let(:dish_2) { double(:dish_2, name: 'Bar stew', price: 6, print_price: '£6.00') }
  subject(:subject) { described_class.new([dish_1, dish_2]) }
  it 'lists available dishes' do
    expect { subject.list }.to output("Foo soup: £2.00\nBar stew: £6.00\n").to_stdout
  end
