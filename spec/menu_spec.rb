...
describe Menu do
  ...
  describe '#add' do
    before { menu.add(dish) }
    it 'adds the dish to the menu' do
      expect(menu.view).to include(dish.name.upcase => dish.price)
    end
    context 'adding a duplicate dish' do
      it 'raises an error' do
        expect{ menu.add(dish) }.to raise_error(Menu::DUPE_ERR)
      end
    end
  end
  ...
  describe '#remove' do
    ...
    context 'dish is on the menu' do
      before { menu.add(dish) ; menu.add(dish2) }
