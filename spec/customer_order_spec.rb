...
let(:order)	{ CustomerOrder.new(menu) }
let(:order1)	{ CustomerOrder.new(menu1) }
	...
	describe "#total_cost"
		...
		it 'will tell the customer how much their order comes to' do
			order.add_to_order("oranges",4)
			expect(order.total_cost).to eq 8
		end
