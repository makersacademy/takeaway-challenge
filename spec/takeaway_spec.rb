...
describe Takeaway do
  ...
  context "intitialized instance variable" do
    ...
    it "menu" do
      expect(subject.instance_variable_get(:@menu)).to eql(menu)
    end
    it "basket" do
      expect(subject.instance_variable_get(:@basket)).to eql(basket)
    end
    it "checkout" do
      expect(subject.instance_variable_get(:@checkout)).to eql(checkout)
    end
  end
  ...
  context "method functionality" do
    it "#show_menu delegates to Menu.print" do
      expect(menu).to receive(:print).once
      subject.show_menu
    end
    ...
    it "#empty_basket delegates to Basket.empty" do
      expect(basket).to receive(:empty).once
  ...
  context "handling error" do
    it "#add_to_basket throws error if requested dish is not on menu" do
      allow(menu).to receive(:dish_exists?).and_return(false)
    ...
    it "#add_to_basket throws error if not enough of the requested dish is available" do
      allow(menu).to receive(:dish_available?).and_return(false)
    ...
    it "#checkout throws error if basket is empty" do
      allow(basket).to receive(:details).and_return([])
