...
describe Takeaway do
  ...
  it 'prints returned menu'do
    ...
    expect{ subject.read_menu }.to output("{\"pizza\"=>1.99}").to_stdout
  end

  it 'prints confirmation for added products' do
    expect{ subject.order("pizza",2) }. to output("2x pizza(s) added to order").to_stdout
  end

  it 'prints rejection for not present products' do
    allow(order).to receive(:add) { false }
    expect{ subject.order("sausage",2) }.to output("Sorry, no sausage available").to_stdout
  end
