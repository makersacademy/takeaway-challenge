...
describe Printer do
  describe '#display_menu' do
    it 'displays name and price of each dish' do
      ...
      expect(subject.display_menu("Fish and Chips", 1295)).to eq(response)
    end
  end
  ...
  describe '#format_order' do
    it 'displays name, price, qty, subtotal' do
      ...
      expect(subject.format_order("Fish and Chips", 1295, 4, 5180)).to eq(response)
    end
  end
  ...
  describe '#format_total' do
    it 'displays grand total for order' do
      ...
      expect(subject.format_total(5180)).to eq(response)
    end
  end
end
