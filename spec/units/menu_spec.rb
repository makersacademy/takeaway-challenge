...
describe Menu do
  ...
  subject(:menu) { described_class.new send_sms_class }
  describe '#initialize' do
    it 'creates an empty array called selection' do
      expect(menu.selection).to eq []
    end
  ...
  describe '#see' do
    it 'enables users to see everything on the menu' do
      menu_text = menu.menuprinter.see_menu menu.dishes
  ...
  describe '#select' do
    it 'enables users to choose an item on the menu and save it to the selection array' do
      menu.select 'Pizza Diavola', 1
      expect(menu.selection[0].name).to eq 'Pizza Diavola'
    end
    it 'enables users to choose a particular number of items' do
      menu.select 'Pizza Diavola', 3
      expect(menu.selection[0].quantity_ordered).to eq 3
    end
    it 'tells a user if they are trying to order something that is not available' do
      expect(menu.select 'Pizza Napolitana', 1).to eq 'Sorry, that is not available'
    end
    it 'enables users to add muliples items' do
      menu.select 'Pizza Diavola', 3
      menu.select 'Tagliatelle with duck ragu', 1
      menu.select 'Salad Caprese', 2
      expect(menu.selection.length).to eq 3
    end
  end
  describe '#check_order' do
    ...
    it 'raises an error if the price supplied by the customer does not match the calculated total price' do
      expect { menu.check_order 30 }.to raise_error "Sorry, our sums do not match"
    end
    it 'returns a string' do
      expect(menu.check_order total_cost).to be_a String
    end
    it 'which contains the names of the dishes ordered' do
      matches = menu.check_order(total_cost).scan 'Pizza Diavola'
      matches += menu.check_order(total_cost).scan 'Tagliatelle with duck ragu'
      expect(matches.length).to be 2
    end
    it 'and contains the total cost of the dishes ordered' do
      expect(menu.check_order total_cost).to include total_cost.to_s
    end
  end
end
