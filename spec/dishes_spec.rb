...
describe Dishes do
  ...
  before(:each) do
    ...
    dishes.instance_variable_set(:@dishes, [@dish1, @dish2])
  ...
  describe '#describe' do
    it 'returns correctly formatted string' do
      expect(dishes.describe).to eq "Menu:\n#{@dish1.describe}\n#{@dish2.describe}"
    end
  ...
  describe '#get_dish' do
    it 'returns the dish with the given id if it exists' do
      expect(dishes.get_dish(1)).to eq @dish1
    end
  
    it 'raises an error if dish not found' do
      expect { dishes.get_dish 3 }.to raise_error Dishes::ERRORS[:no_dish]
    end
