...
describe Customer do
  ...
  let(:twilio) { double :twilio, send_message: "Thank you. Your order has been placed and will be delivered before #{(time.hour + 1).to_s}:#{time.min}"}
  ...
  it 'should be able to send a text verifying correct order has been placed' do
    time = Time.new
    expect(subject.confirm_order twilio).to eq("Thank you. Your order has been placed and will be delivered before #{(time.hour + 1).to_s}:#{time.min}")
  end
