...
describe Order do
  ...
  describe "#add_dish" do
    ...
    it "adds multiple dishes to an order" do
      order.add_dish(dish, quantity_two)
      order.add_dish(dish_two, quantity_one)
    ...
    it "raises a error with negative quantities" do
      expect{ order.add_dish(dish, quantity_minus_one) }.to raise_error "Quantity ordered needs to be greater than zero"  
    end

    it "raises an error if zero used as a quantity" do
      expect{ order.add_dish(dish, quantity_zero) }.to raise_error "Quantity ordered needs to be greater than zero"  
    end
  end
