...
describe Order do
  ...
  before do
    allow(menu).to receive(:available?).with(:cheeseburger).and_return(true)
    allow(menu).to receive(:available?).with(:hamburger).and_return(true)
    allow(menu).to receive(:available?).with(:fries).and_return(true)
  ...
  it "should not add items to the order that are not on the menu" do
    allow(menu).to receive(:available?).with(:noodles).and_return(false)
