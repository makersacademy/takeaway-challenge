...
describe Order do
  ...
  describe "#display" do
    it 'displays the order on the screen' do
      ...
      line1 = "-"*(spacing-9) + "\n"
      line2 = "ORDER\n\n"
      line3 = "Item".ljust(spacing/2) + "Price".ljust((spacing-34)/2)+ "Quantity".ljust(spacing) +  "\n"
      line4 = "----".ljust(spacing/2) + "-----".ljust((spacing-34)/2)+ "--------".ljust(spacing) + "\n"
      line5 = "Pollo".ljust(spacing/2)+(("£10.00").ljust((spacing-34)/2))+ "x2".ljust(spacing)
      line6 = "\n" + ("-"*(spacing-9)) + "\n"
