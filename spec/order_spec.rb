...
describe Order do
  ...
  describe '#add dishes' do
    ...
    # is to make it so the test will adjust even if availability of that item changes (i.e. if the hash is changed so that item 3 is now
    # available) # what we want is a test that imagines it as unavailable without ruining the rspec results (I say this, as if I don't
    # already have two failing tests). So I guess this means doubling...
  ...
  # the stubbing above is from the same attempt to require input from the user (i.e. type 'checkout') but didn't work
...
# cannot make the above work! works in IRB, but can't get a passing test... also, there was a point at which putting in my number
# seemed to allow the tests to escape the stub in the sms_spec at times, although the above doesn't send a text so I think this is fixed...
    context 'when a customer has checked their summary and enters complete_order' do
...
# another attempt to check that the complete_order method instantiated an instance of SMS, but as you can see, it also failed
