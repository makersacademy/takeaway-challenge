...
class Menu{
  constructor(){
    this.order = [];
    this.items = [
     {"item": "fish and chips", "price": 4.99,
      "quantity": 0, "totalPrice": 0},
     {"item": "deep fried pizza", "price":
      3.99, "quantity": 0, "totalPrice": 0},
     {"item": "battered sausage", "price": 2.99,
      "quantity": 0, "totalPrice": 0},
     {"item": "battered white pudding", "price": 2.50,
      "quantity": 0, "totalPrice": 0},
     {"item": "deep fried mars bar", "price": 1.50,
      "quantity": 0, "totalPrice": 0},
     {"item": "Irn Bru 1.5l bottle", "price": 1.99,
      "quantity": 0, "totalPrice": 0}
   ];
  }
    increase(item){
      ...
      receipt(){
        ...
        for (var i = 0; i <this.items.length; i++){
          if (this.items[i].quantity > 0){
            ...
            let food = this.items[i].item;
            let price = this.items[i].price * quantity;
             this.order.push({"food": food, "quantity": quantity, "price": +price.toFixed(2)});
           }
          }   return this.order;
         }
